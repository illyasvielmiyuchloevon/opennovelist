...
class WorkflowSubmissionPayload(BaseModel):
    content_md: str = Field(
        "",
        validation_alias=AliasChoices("content_md", "contentMd", "content"),
        description="阶段摘要或 Markdown 结果；通常用于兼容旧调用。",
    )
    chapter_txt: str = Field(
        "",
        validation_alias=AliasChoices("chapter_txt", "chapterTxt", "chapter", "chapter_text", "chapterText"),
        description="章节正文兼容字段；新 agent 阶段应优先用文档工具写入正文文件。",
    )
    ...
    review_md: str = Field(
        "",
        validation_alias=AliasChoices("review_md", "reviewMd", "review"),
        description="审核 Markdown 正文。",
    )
    blocking_issues: list[str] = Field(
        default_factory=list,
        validation_alias=AliasChoices("blocking_issues", "blockingIssues"),
        description="阻塞问题列表。",
    )
    rewrite_targets: list[str] = Field(
        default_factory=list,
        validation_alias=AliasChoices("rewrite_targets", "rewriteTargets"),
        description="需要重写或更新的目标。",
    )
    chapters_to_revise: list[str] = Field(
        default_factory=list,
        validation_alias=AliasChoices("chapters_to_revise", "chaptersToRevise"),
        description="需要返工的章节编号列表。",
    )
    generated_files: list[str] = Field(
        default_factory=list,
        validation_alias=AliasChoices("generated_files", "generatedFiles", "files"),
        description="本阶段已生成或更新的 file_key 列表。",
    )
