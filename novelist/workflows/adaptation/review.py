    ...
    *,
    ...
    return {
        ...
        "review_scope": {
            ...
            "document_set_policy": (
                "审核下游仿写实际会用到的完整当前资料集。第 001 卷应包含 5 个核心资料文档；"
                "后续卷审核本卷更新文档，并带上已存在的文笔写作风格文档。"
            ),
        },
        "requirements": [
            "所有被审核的资料文档主体都必须是新书资料：必须描述新书世界、新书人物/势力/地点/事件/术语和目标世界观下的设定；参考源只允许作为功能映射依据，不得作为新书正文事实、设定正文、剧情正文或话语体系出现。",
            "审核时必须区分“参考源侧功能映射”和“新书设定主体”：明确标注在参考源功能映射、参考源侧、原书功能等语境中的参考源人物/地名/事件/术语可以出现，不应仅因出现参考源内容就判定不通过；只有它们被写成新书设定主体、正文事实或新书话语体系时才算污染。",
            "必须交叉检查 adaptation_documents 中所有资料的一致性：同一新书世界、时间线、故事方向、实体命名、势力关系、地点体系、修炼体系、资源规则和术语体系必须前后一致；不能一处使用新书设定、一处沿用参考源设定，或不同文档互相矛盾。",
            "如果任一资料文档在非功能映射语境中仍以参考源世界、参考源人物、参考源地名、参考源事件或参考源专用话语体系描述新书主体，必须判定不通过，并把对应 file_key 写入 rewrite_targets。",
            "判断资料是否足够支撑后续章节仿写，而不是只检查格式是否完整。",
            "检查参考源人物名、地名、姓氏、势力名、事件名称、专用术语、等级体系、称谓口吻、标志性台词和话语体系在新书设定主体中是否已经替换或映射，不得直接照搬；但不要把明确标注的参考源侧功能映射误判为污染。",
            "检查世界模型是否已经合并承载目标世界观、背景故事和世界知识，并与参考源明显区分；故事类型、角色功能位、剧情结构和人物推进不属于世界模型。",
            ...
            "检查世界模型是否没有把故事类型、角色功能位、卷内剧情衔接、人物关系推进、主角状态等非世界知识内容塞入世界模型；默认标题不要求全部填满，不能因为空栏目或未填栏目判定失败。",
            "检查全书大纲是否是仿写书籍的大纲，不得把参考源原大纲照抄为新书大纲。",
            "检查全书大纲是否保持卷级粒度；旧卷的“卷末阶段状态与衔接”不应长期逐卷保留，整份文档只应保留当前最新卷的该小节，旧卷衔接应被下一卷大纲承接或删除。",
