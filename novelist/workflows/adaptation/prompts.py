...
def world_model_scope_text() -> str:
    ...
    return (
        ...
        "也只沉淀到当前卷为止后续章节会反复引用、会影响规则判断或设定一致性的稳定世界知识；完整卷原文只是判断依据，不是待抽取清单。默认按 16 个二级标题组织："
        f"{section_text}。二级标题下只在确有稳定设定类别时展开少量必要子条目；不要为了填满栏目而罗列当前卷事实。"
        "世界模型只允许写世界观与世界知识：背景、历史源流、地点、势力、制度、能力体系、资源、规则、术语、世界真相和常识。"
            ...
            stable_fields={
                ...
                "requirements": [
                    *source_contamination_guardrails(),
                    ...
                    "世界知识放世界模型，剧情推进放全书大纲、卷级大纲或卷级剧情进程，故事线连续性放全书故事线蓝图；不要把这些内容伪装成伏笔。",
            ...
            stable_fields={
                ...
                "requirements": [
                    *source_contamination_guardrails(),
                    ...
                    "这是全书级世界模型文档，并合并承载世界观设计职责；采用按卷增量维护方式，每卷只补充、修正当前卷揭示出的长期有效世界知识与世界观设定。",
                    ...
                    "世界模型严禁记录卷内已发生大事件、主角个人战绩、考试名次、榜单变化、竞技活动结果、家庭债务或治疗进度、奖励记录、角色关系进展、剧情推进清单；这些内容应属于全书大纲、卷级大纲、卷级剧情进程、章节状态或故事线蓝图，不属于世界模型。",
                    "如果当前文件中已有“卷XXX内已发生/已公开大事件”“本卷剧情进展”“主角战绩记录”等非世界知识小节，本次必须用 edit/patch 将其移出世界模型语义，改为只保留由这些事件揭示出的世界规则、制度、地点、势力、资源、术语或公开常识。",
                    "世界模型需要承载原世界观设计阶段独有职责：目标世界观设定、背景故事、能力设计、道具设计、势力设计、角色功能位、故事类型与原书功能映射；但只在这些职责确有本卷新增或修正时补写。",
                    "默认使用 scope 中给出的 16 个二级标题组织世界模型；如果某些栏目当前卷暂无稳定信息，可以保留标题而不填充细节，但不要删除默认标题。",
