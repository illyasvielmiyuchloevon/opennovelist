...
PUBLIC_RUN_MODES = (RUN_MODE_CHAPTER, RUN_MODE_GROUP, RUN_MODE_VOLUME)
CHAPTER_AFTER_ACTION_NEXT = "next"
CHAPTER_AFTER_ACTION_RESELECT_MODE = "mode"
CHAPTER_AFTER_ACTION_EXIT = "exit"
...
def chapter_phase_uses_source_bundle(phase_key: str) -> bool:
    return str(phase_key or "").strip() in CHAPTER_SOURCE_BUNDLE_PHASES
...
def normalize_rewrite_run_mode(run_mode: str, *, warn: bool = True) -> str:
    ...
    if normalized in PUBLIC_RUN_MODES:
        return normalized
    fail(f"不支持的章节重写运行模式：{run_mode}。支持：chapter、group、volume。")
...
HEADING_MANAGED_DOC_SPECS = {
    "character_status_cards": {
        ...
        "section_policy": [
            ...
            "这是一份人物当前状态快照，不是章节回顾、逐章变化日志或剧情流水账。",
        ],
        "update_rules": [
            "只更新受当前章节影响、并且此刻仍然成立的人物状态、目标、压力和关键变量。",
            "如果本章带来了新变化，必须把变化折叠进对应人物的当前状态描述，直接改写旧条目，不要按章节号逐条累积。",
            "不要写“第00xx章补充 / 推进 / 变化”这类逐章记录；已经失效或不再关键的历史过程应压缩为简短背景。",
            "单个人物条目以当前状态结论为主，不要把多章剧情按时间顺序整段复述进同一条。",
    ...
    "character_relationship_graph": {
        ...
        "section_policy": [
            ...
            "这是一份人物关系当前快照，不是逐章关系推进日志。",
        ],
        "update_rules": [
            "只更新受当前章节影响、并且此刻仍然成立的人物关系状态、张力与潜在冲突。",
            "如果本章带来了关系变化，必须把变化折叠进对应关系的当前描述，直接改写旧条目，不要按章节号逐条累积。",
            "不要写“第00xx章推进 / 补充”这类关系流水账；已经兑现、缓解或失效的阶段性变化应合并进当前关系结论。",
            "单条关系应优先说明当前关系是什么、为什么重要、目前有哪些风险，不要把多章互动按时间顺序复述成长段纪要。",
    ...
    "foreshadowing": {
        ...
        "section_policy": [
            ...
            "这是一份仍然有效的伏笔索引，不是每章剧情回顾、章节变化日志或流水账。",
        ],
        "update_rules": [
            "只在当前章节出现新的伏笔埋设、已有伏笔状态发生关键推进、或伏笔被正式回收时更新。",
            "普通训练进度、考试排名、购物补给、一次性冲突、常规修炼反馈等章节内容，若不构成伏笔埋设/推进/回收，不要写入伏笔文档。",
            "每条伏笔只保留当前仍有效的埋设状态、最新关键推进或回收结果；不要按章节号把推进过程逐条累积成流水账。",
            "不要写“第00xx章推进 / 补充 / 回收记录”式的长串章节纪要；如需标注来源，只保留首次埋设、最近关键推进或正式回收的少量关键章节号。",
    ...
    "world_state": {
        ...
        "section_policy": [
            ...
            "这是一份世界当前状态快照，不是按章节堆积的事件回顾或主角经历流水账。",
        ],
        "update_rules": [
            "只更新受当前章节影响、并且此刻仍然成立的世界层面变化，例如公共规则、城域局势、地点功能、势力关系、社会认知或公开事件后果。",
            "主角个人训练、单次战斗、购物补给、成绩波动等内容，只有在它们真实改变了世界层面的公开状态或长期结构时才可写入。",
            "如果当前章节带来了世界变化，必须把变化折叠进对应地点、势力、规则或局势条目的当前描述，不要按章节号逐条累积成长篇纪要。",
            "不要写“第00xx章状态变化 / 推进”式章节流水账；已经失效、不再关键或仅用于复述剧情的过程应压缩删除。",
