...
它面向“卷级规划层”。卷资料审核通过后不会立刻把本卷加入 `processed_volumes`，而是继续进入整卷组纲计划生成、组纲落盘和组纲审核；只有组纲审核通过后，本卷资料适配才算完成。
...
负责按已审核组纲计划里的动态章节组重写与审核；分组唯一来源是 `group_injection/<volume>_group_injection/00_group_outline_plan.md`，不再按固定 5 章切组。包括：
...
它面向“章节生产层”。章节组生成、组审和卷级审核只读取已审核组纲、卷级注入、全局注入和已生成正文，不再加载参考源章节正文；组纲在章节阶段只读冻结，组纲问题需要回到卷资料适配的组纲审核阶段修正。
...
### 3.4 `novelist.core.group_outline_plan`

整卷组纲计划与路径工具：

- `group_injection/<volume>_group_injection/00_group_outline_plan.md` 的读写
- 动态章节组编号、目录和组纲/组审路径计算
- 组纲计划审核状态校验
- 供章节工作流读取已审核组纲列表

### 3.5 `novelist.core.openai_config`
...
### 3.6 `novelist.core.responses_runtime`
...
当前对模型暴露的工具总共 **5 个**；大多数 agent 阶段使用通用 4 个工具，组纲计划阶段额外使用一个结构化计划提交工具。

### 4.1 `submit_group_outline_plan`

只用于 `novel_adaptation` 的整卷组纲计划阶段。

用途：

- 提交本卷动态章节组数量
- 提交每组新书章节数、对应参考源章节范围和推进功能指导
- 写入 `00_group_outline_plan.md` 的计划草案

对应代码：

- [novelist/workflows/adaptation/group_outlines.py](../novelist/workflows/adaptation/group_outlines.py)

### 4.2 `submit_workflow_result`
...
除组纲计划阶段外，当前两个主工作流阶段统一暴露同一组 4 个通用工具：
...
- `novel_adaptation` 整卷组纲生成阶段：同一个 agent 会话根据已提交的动态组计划写出本卷所有组纲
- `novel_adaptation` 组纲审核阶段：审核整卷所有组纲，允许原地返修组纲文件，审核通过后才标记本卷适配完成
- `novel_chapter_rewrite` 组生成阶段：同一个 agent 会话读取当前已审核组纲，生成当前组正文和状态文档；组纲是只读规划输入
- 章节组审 / 卷审阶段：允许先返修章节正文、状态文档、审核文档，再提交审核结论；不改写已审核组纲
...
- 在新组生成路径中，它和 `document_request`、已审核 `current_group_outline`、`requirements` 一起定义“这一组怎么写”
...
1. 识别输入类型
2. 如需要，先 `split_novel`
3. 运行 `novel_adaptation`：卷资料生成 -> 卷资料审核 -> 整卷组纲生成 -> 组纲审核
4. 再运行 `novel_chapter_rewrite`：章节组正文生成/组审 -> 卷级章节审核
5. 结束后回到统一入口菜单
