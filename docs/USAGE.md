...
- 默认最多 50 章一卷。
- 每卷会按工作流实际注入的 source bundle 估算字符数；超过 150k 字符时，自动把卷尾章节顺延到下一卷。
- 单章本身超过 150k 时会单章成卷并打印警告。
- `novel_adaptation` 启动后也会在 API 请求前检查当前未完成卷及后续卷；已完成卷资料适配的卷不再拆，未完成卷会自动备份并重排。
...
卷目录可能不是严格每 50 章：当某卷预计注入字符数超过 150k 时，系统会自动减少该卷章数，并把多余章节顺延到后续卷。运行时自动重分卷会在源目录下写入 `.source_rebalance_backups/`，并在工程目录下写入 `source_rebalance_backups/` 保存受影响的未完成产物。
...
- 新建文档时允许整篇写入
- 已存在的长期知识文档，按修改意图选择 edit 或 patch
- 尽量保留未变化内容
- 只有必要时才更新对应文档
- 全局资料只保留长期复用的索引、规则和映射，不写成逐章流水账
...
- `伏笔文档`
  是唯一的全局伏笔文档。资料适配阶段只更新全书级/卷级伏笔设计索引，保留参考源功能映射、新书伏笔设计、埋设意图、适用范围和后续呼应方向；章节工作流写入的推进、兑现或闭合记录必须原样保留，后续卷资料适配不得删改这些运行时记录
- `人物状态卡 / 人物关系链 / 世界状态`
