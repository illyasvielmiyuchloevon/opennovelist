...
- `全书故事线蓝图`
  现在由 `novelist.workflows.novel_adaptation` 按故事线与分卷蓝图区块增量维护；每卷只记录该卷在全书故事线中的功能、主要转折方向、跨卷衔接和后续约束，不记录章内事件、场景清单或逐章剧情
- `伏笔文档`
...
章节重写请求会按文档类型裁剪注入内容。即使已有全局文档偏大，发送给模型的内容也会被控制在对应预算内，避免伏笔、故事线、世界模型等文档把上下文顶爆。
